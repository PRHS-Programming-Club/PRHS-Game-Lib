...
namespace PRHS {
	...
	public:
		...
		int size() const; //Get the number of frames in the animation

		std::string next(); //Get the next frame in the animation
		std::string getFrame(const int position); //Get a certain frame

		int currentPosition() const; //Returns the index of the next frame that will be returned by next() (First frame returns zero)
		...
		int getUpdateTime() const; //Returns the current update time
